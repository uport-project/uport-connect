...
class Connect {
  ...
  constructor(appName, opts = {}) {
    ...
    this.mobileTransport =
      opts.mobileTransport ||
      transport.url.send({
        uriHandler: opts.mobileUriHandler,
        messageToURI: m =>
          this.useDeeplinks ? message.util.messageToDeeplinkURI(m) : message.util.messageToUniversalURI(m),
      })
    this.onloadResponse = opts.onloadResponse || transport.url.getResponse()
    this.pushTransport =
      this.pushToken && this.publicEncKey ? pushTransport(this.pushToken, this.publicEncKey) : undefined
  ...
  async sendTransaction(txObj = {}, id, sendOpts) {
    ...
    this.credentials
      .createTxRequest(txObj, { callbackUrl: this.genCallback(id) })
  ...
  async requestVerificationSignature(unsignedClaim, sub, id = 'verSigReq', sendOpts) {
    ...
    this.credentials
      .createVerificationSignatureRequest(unsignedClaim, {
        sub,
        aud: this.did,
  ...
  requestTypedDataSignature(typedData, id = 'typedDataSigReq', sendOpts) {
    this.credentials
      .createTypedDataSignatureRequest(typedData, { riss: this.did, callback: this.genCallback(id) })
      .then(jwt => this.send(jwt, id, sendOpts))
  }
  ...
  async requestDisclosure(reqObj = {}, id = 'disclosureReq', sendOpts) {
    ...
    this.credentials.createDisclosureRequest(reqObj, reqObj.expiresIn).then(jwt => this.send(jwt, id, sendOpts))
  }
  ...
  signAndUploadProfile(profile) {
    ...
    return this.credentials
      .createVerification({ sub: this.keypair.did, claim: profile })
...
const pushTransport = (pushToken, publicEncKey) => {
  ...
  return (request, { type, redirectUrl, data }) => {
    if (transport.messageServer.isMessageServerCallback(request)) {
      return transport.messageServer
        .URIHandlerSend(send)(request, { type, redirectUrl })
        .then(res => {
          transport.ui.close()
...
function ipfsAdd(jwt) {
  return new Promise((resolve, reject) => {
    ...
    req.onreadystatechange = () => {
      ...
      if (req.status !== 200) reject(new Error(`Error ${req.status}: ${req.responseText}`))
